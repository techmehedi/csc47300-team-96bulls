class AuthenticationSystem {
    ...
    async handleSignup(e) {
        ...
        const formData = {
            ...
            accountType: form.accountType?.value || 'user'
        };
        // Basic validation
        ...
        try {
            ...
            if (user) {
                ...
                // Create user role in database via backend API
                try {
                    console.log(`Creating user role: userId=${user.id}, accountType=${formData.accountType}`);
                    const apiBase = (window.API_URL || 'http://localhost:3000/api').replace(/\/$/, '');
                    const response = await fetch(`${apiBase}/users/role`, {
                        method: 'POST',
                        headers: {
                    ...
                    // Continue anyway - role can be set manually later
                }
            ...
            this.showNotification(requiresEmailConfirm ? 'Account created! Check your email to confirm.' : 'Account created successfully! Redirecting...', 'success');
