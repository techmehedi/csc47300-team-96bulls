...
interface TestWindow extends Window {
  supabase?: any;
  supabaseClient?: FullSupabaseClient;
  SUPABASE_URL?: string;
...
describe('Authentication System', () => {
  ...
  beforeEach(() => {
    ...
    // Mock window objects
    (window as any).supabaseClient = mockSupabaseClient;
    (window as any).SUPABASE_URL = 'https://test.supabase.co';
    (window as any).SUPABASE_ANON_KEY = 'test-anon-key';
  });
  ...
  describe('Error Handling', () => {
    test('should handle missing Supabase client', () => {
      ...
      (window as any).supabaseClient = undefined;

      // Should gracefully handle missing client
      expect(window.supabaseClient).toBeUndefined();

      (window as any).supabaseClient = originalClient;
    });
