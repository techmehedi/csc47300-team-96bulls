...
(function(): void {
  ...
  let overlay = document.getElementById('page-transition-overlay') as HTMLElement | null;
  if (!overlay) {
  ...
  const handleLinkClick = (e: Event): void => {
    ...
    if (overlay) {
      ...
      const loader = overlay.querySelector<HTMLElement>('.page-loader');
      if (loader) {
  ...
  const attachListeners = (): void => {
    const links = document.querySelectorAll<HTMLAnchorElement>('a[href]:not([href^="#"]):not([data-no-transition])');
    links.forEach(link => {
