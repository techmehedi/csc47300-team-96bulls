...
import type { Session, Progress, Stats, SessionResult, User, FullSupabaseClient } from './types.js';
...
class DashboardSystem {
  ...
  private userStats: Stats | null = null;
    ...
    if (window.location.hash) {
      ...
      if (accessToken && refreshToken && window.supabaseClient) {
        ...
        try {
          ...
          const { data, error } = await (client.auth as any).setSession({
          ...
          if (data?.user) {
            ...
            // Store user session
            this.currentUser = this.mapSupabaseUser(data.user as User);
  ...
  initializeEventListeners(): void {
    ...
    const topicCards = document.querySelectorAll<HTMLElement>('.topic-card');
    ...
    const goalItems = document.querySelectorAll<HTMLElement>('.goal-item');
      ...
      if (window.backendAPI) {
        try {
          ...
          const allNetworkErrors = allFailed && 
            (statsData.reason as NetworkError)?.isNetworkError &&
            (progressData.reason as NetworkError)?.isNetworkError &&
            (sessionsData.reason as NetworkError)?.isNetworkError;
          ...
          if (window.supabaseClient && supabaseDB) {
            try {
              
              // Load from Supabase in parallel for faster loading
              const [statsData, progressData, sessionsData] = await Promise.allSettled([
                supabaseDB.getUserStats(userId),
                supabaseDB.getUserProgress(userId),
                supabaseDB.getUserSessions(userId)
              ]);
  ...
  updateStatsCards(): void {
    ...
    solvedEl.textContent = String(totalSolved);
  ...
  updateProgressChart(sessions: Session[]): void {
  ...
  normalizeResults(results: SessionResult[] | string | any): SessionResult[] {
    if (!results) return [];
  ...
  prepareDailyData(sessions: Session[], dates: Date[]): number[] {
  ...
  updateTopicPerformance(userProgress: Progress[]): void {
    const topicsGrid = document.getElementById('topicsGrid') || document.querySelector<HTMLElement>('.topics-grid');
    if (!topicsGrid) {
    ...
    const topicConfig: Record<string, { icon: string; name: string }> = {
    ...
    const topicStats: Record<string, { totalAttempted: number; totalCorrect: number; accuracy: number }> = {};
  ...
  updateRecentActivity(sessions: Session[]): void {
  ...
  updateGoals(): void {
    const goalsList = document.querySelector<HTMLElement>('.goals-list');
    if (!goalsList || !this.userStats) return;

    // Update existing goals with real data
    const goalItems = goalsList.querySelectorAll<HTMLElement>('.goal-item');
    
    // Goal 1: Solve 50 Problems
    if (goalItems[0]) {
      const totalSolved = this.userStats.totalSolved || 0;
      const progress = Math.min((totalSolved / 50) * 100, 100);
      const progressBar = goalItems[0].querySelector<HTMLElement>('.progress-fill');
      const progressText = goalItems[0].querySelector<HTMLElement>('.goal-progress span');
      
      if (progressBar) progressBar.style.width = `${progress}%`;
      if (progressText) progressText.textContent = `${totalSolved}/50`;
      
      const goalContent = goalItems[0].querySelector<HTMLElement>('.goal-content p');
      if (goalContent) {
        if (totalSolved === 0) {
    ...
    if (goalItems[1]) {
      ...
      const progressBar = goalItems[1].querySelector<HTMLElement>('.progress-fill');
      const progressText = goalItems[1].querySelector<HTMLElement>('.goal-progress span');
      
      if (progressBar) progressBar.style.width = `${progress}%`;
      if (progressText) progressText.textContent = `${currentStreak}/${targetStreak}`;
      
      const goalContent = goalItems[1].querySelector<HTMLElement>('.goal-content p');
      if (goalContent) {
    ...
    if (goalItems[2]) {
      ...
      const progressBar = goalItems[2].querySelector<HTMLElement>('.progress-fill');
      const progressText = goalItems[2].querySelector<HTMLElement>('.goal-progress span');
      
      if (progressBar) progressBar.style.width = `${progress}%`;
      if (progressText) progressText.textContent = `${currentAccuracy}%`;
      
      const goalContent = goalItems[2].querySelector<HTMLElement>('.goal-content p');
      if (goalContent) {
  ...
  navigateToPractice(topic: string): void {
    // Store selected topic in localStorage for practice session
    localStorage.setItem('ai_interviewer_selected_topic', topic);
    window.location.href = 'practice.html';
  }

  showGoalDetails(goalItem: HTMLElement): void {
    const goalTitle = goalItem.querySelector('h4');
    if (goalTitle) {
      this.showNotification(`Goal: ${goalTitle.textContent}`, 'info');
    }
  }
  ...
  setupErrorHandling(): void {
    ...
    window.addEventListener('unhandledrejection', (event: PromiseRejectionEvent) => {
      ...
      if (error && typeof error === 'object') {
        ...
        if (networkError.handled === true && networkError.isNetworkError === true) {
