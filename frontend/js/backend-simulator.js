class BackendSimulator {
    ...
    async updateUserProgress(userId, session) {
        ...
        let progress = this.data.progress.find(p => p.userId === userId && p.topic === topic && p.difficulty === difficulty);
        if (!progress) {
            progress = {
    ...
    async getUserStats(userId) {
        ...
        const totalSolved = sessions.reduce((sum, session) => sum + session.results.filter(r => r.isCorrect).length, 0);
    ...
    async getQuestions(topic, difficulty, limit = 10) {
        const questions = this.data.questions.filter(q => q.topic === topic && q.difficulty === difficulty);
...
//# sourceMappingURL=backend-simulator.js.map